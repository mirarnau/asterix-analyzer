...
export async function adapt(message : Buffer, id : number) : Promise<Cat21> {
  ...
  schduler.addOperation(cat21.setDataSourceIdentifier(dataSourceIdentifier));
  offset += 2;

  //Target Report Descriptor
  let len = operator.getVariableItemOffset(message.subarray(offset, offset + 3), 3);
  var targetReportDescriptor = message.subarray(offset, offset + len);
  schduler.addOperation(cat21.setTargetReporDesriptor(targetReportDescriptor))
  offset += len;

  // Track Number
  if (fspec[2] === "1") {
    var trackNumber = message.subarray(offset, offset + 2);
    schduler.addOperation(cat21.setTrackNumber(trackNumber));
    offset += 2;
  }
  // Service Identification
  if (fspec[3] === "1") {
    var serviceIdentification = message.subarray(offset, offset + 1);
    schduler.addOperation(cat21.setServiceIdentification(serviceIdentification));
    offset += 1;
  }
  // Time of Applicability for Position
  if (fspec[4] === "1") {
    var timeofApplicabilityforPosition = message.subarray(offset, offset + 3);
    schduler.addOperation(cat21.setTimeofApplicabilityforPosition(timeofApplicabilityforPosition));
    offset += 3;
  }
  // Position in WGS-84 coordinates
  if (fspec[5] === "1") {
    var positioninWGS84Coordinates = message.subarray(offset, offset + 6);
    schduler.addOperation(cat21.setPositioninWGS84Coordinates(positioninWGS84Coordinates));
    offset += 6;
  }
  ...
  if (fspec[6] === "1") {
    ...
    schduler.addOperation(cat21.setHighResPositionPositioninWGS84Coordinates(highResolutionPositioninWGS84Coordinates));
    offset += 8;
  }
  // FX
  if (fspec[7] === "1") {      
    ...
    if (fspec[8] === "1") {
      ...
      schduler.addOperation(cat21.setTimeofApplicabilityforVelocity(timeofApplicabilityforVelocity));
      offset += 3;
    }
    // Air Speed
    if (fspec[9] === "1") {
      var airSpeed = message.subarray(offset, offset + 2);
      schduler.addOperation(cat21.setAirSpeed(airSpeed));
      offset += 2;
    }
    // True Air Speed
    if (fspec[10] === "1") {
      var trueAirSpeed = message.subarray(offset, offset + 2);
      schduler.addOperation(cat21.setTrueAirSpeed(trueAirSpeed));
      offset += 2;
    }
    /// Target Address
    var targetAddress = message.subarray(offset, offset + 3);
    schduler.addOperation(cat21.setTargetAddress(targetAddress));
    offset += 3;
    
    // Time of Message Reception of Position
    if (fspec[12] === "1") {
      var timeofMessageReceptionofPosition = message.subarray(offset, offset + 3);
      schduler.addOperation(cat21.setTimeofMessageReceptionofPosition(timeofMessageReceptionofPosition));
      offset += 3;
    }
    // Time of Message Reception of Position-High
    if (fspec[13] === "1") {
      var timeofMessageReceptionofPosition_highPres = message.subarray(offset, offset + 4);
      schduler.addOperation(cat21.setTimeofMessageReceptionofPositionHighPres(timeofMessageReceptionofPosition_highPres));
      offset += 4;
    }
    // Time of Message Reception of Velocity
    if (fspec[14] === "1") {
      var timeofMessageReceptionofVelocity = message.subarray(offset, offset + 3);
      schduler.addOperation(cat21.setTimeofMessageReceptionofVelocity(timeofMessageReceptionofVelocity));
      offset += 3;
    }
    // FX
    if (fspec[15] === "1") {
      ...
      if (fspec[16] === "1") {
        ...
        schduler.addOperation(cat21.setTimeofMessageReceptionofVelocity_HighPres(timeofMessageReceptionofVelocity_highPres));
        offset += 4;
      }
      // Geometric Height
      if (fspec[17] === "1") {
        var geometricHeight = message.subarray(offset, offset + 2);
        schduler.addOperation(cat21.setGeometricHeight(geometricHeight));
        offset += 2;
      }
      // Quality Indicators
      let len = operator.getVariableItemOffset(message.subarray(offset, offset + 4), 4);
      var qualityIndicator = message.subarray(offset, offset + len);
      schduler.addOperation(cat21.setQualityIndicators(qualityIndicator));
      offset += len;
      // MOPS Version
      if (fspec[19] === "1") {
        var mopsVersion = message.subarray(offset, offset + 1);
        schduler.addOperation(cat21.setMOPSVersion(mopsVersion));
        offset += 1;
      }
      // Mode 3/A Code
      if (fspec[20] === "1") {
        var mode3A = message.subarray(offset, offset + 2);
        schduler.addOperation(cat21.setMode3A(mode3A));
        offset += 2;
      }
      // Roll Angle
      if (fspec[21] === "1") {
        var rollAngle = message.subarray(offset, offset + 2);
        schduler.addOperation(cat21.setRollAngle(rollAngle));
        offset += 2;
      }
      // Flight Level
      if (fspec[22] === "1") {
        var flightLevel = message.subarray(offset, offset + 2);
        schduler.addOperation(cat21.setFlightLevel(flightLevel));
        offset += 2;
      }
      // FX
      if (fspec[23] === "1") {
        ...
        if (fspec[24] === "1") {
          ...
          schduler.addOperation(cat21.setMagneticHeading(magneticHeading));
          offset += 2;
        }
        // Target Status
        if (fspec[25] === "1") {
          var targetStatus = message.subarray(offset, offset + 1);
          schduler.addOperation(cat21.setTargetStatus(targetStatus));
          offset += 1;
        }
        // Barometric Vertical Rate
        if (fspec[26] === "1") {
          var barometricVerticalRate = message.subarray(offset, offset + 2);
          schduler.addOperation(cat21.setBarometricVerticalRate(barometricVerticalRate));
          offset += 2;
        }
        // Geometric Vertical Rate
        if (fspec[27] === "1") {
          var geometricVerticalRate = message.subarray(offset, offset + 2);
          schduler.addOperation(cat21.setGeometricVerticalRate(geometricVerticalRate));
          offset += 2;
        }
        // Airborne Ground Vector
        if (fspec[28] === "1") {
          var airborneGroundVector = message.subarray(offset, offset + 4);
          schduler.addOperation(cat21.setAirborneGroundVector(airborneGroundVector));
          offset += 4;
        }
        // Track Angle Rate
        if (fspec[29] === "1") {
          var trackAngleRate = message.subarray(offset, offset + 2);
          schduler.addOperation(cat21.setTrackAngleRate(trackAngleRate));
          offset += 2; 
        }
        // Time of Report Transmission
        if (fspec[30] === "1") {
          var timeofReportTransmission = message.subarray(offset, offset + 3);
          schduler.addOperation(cat21.setTimeofReportTransmission(timeofReportTransmission));
          offset += 3;
        }
        // FX
        if (fspec[31] === "1") {
          ...
          if (fspec[32] === "1") {
            ...
            schduler.addOperation(cat21.setTargetIdentification(targetIdentification));
            offset += 6;
          }
          // Emitter Category
          if (fspec[33] === "1") {
            var emitterCategory = message.subarray(offset, offset + 1);
            schduler.addOperation(cat21.setEmitterCategory(emitterCategory));
            offset += 1;
          }
          // Met Information
          if (fspec[34] === "1") {
            ...
            schduler.addOperation(cat21.setMetInformation(metInformation, selection));
            offset += selection.length * 2 + len;
          }
          // Selected Altitude
          if (fspec[35] === "1") {
            var selectedAltitude = message.subarray(offset, offset + 2);
            schduler.addOperation(cat21.setSelectedAltutude(selectedAltitude));
            offset += 2;
          }
          // Final State Selected Altitude
          if (fspec[36] === "1") {
            var finalStateSelectedAltitude = message.subarray(offset, offset + 2);
            schduler.addOperation(cat21.setFinalStateSelectedAltitude(finalStateSelectedAltitude));
            offset += 2;
          }
          // Trajectory Intent
          if (fspec[37] === "1") {
            ...
            schduler.addOperation(cat21.setTrajectoryIntent(trajectoryIntent, tis, tid, rep));
            offset += len + 1;
          }
          // Service Management
          if (fspec[38] === "1") {
            var serviceManagement = message.subarray(offset, offset + 1);
            schduler.addOperation(cat21.setServiceManagement(serviceManagement));
            offset += 1;
          }
          // FX
          if (fspec[39] === "1") {
            ...
            if (fspec[40] === "1") {
              ...
              schduler.addOperation(cat21.setAircraftOperationalStatus(aircraftOperationalStatus));
              offset += 1;
            }
            // Surface Capabilities and Characteristics
            if (fspec[41] === "1") {
              let len = await operator.getVariableItemOffset(message.subarray(offset, offset + 2), 2);
              var surfaCapabilitiesandCharacteristics = message.subarray(offset, offset + len);
              schduler.addOperation(cat21.setSurfaceCapabilitiesandCharacteristics(surfaCapabilitiesandCharacteristics));
              offset += len;
            }
            // Message Amplitude
            if (fspec[42] === "1") {
              var messageAmplitude = message.subarray(offset, offset + 1);
              schduler.addOperation(cat21.setMessageAmplitude(messageAmplitude));
              offset += 1;
            }
            // Mode S MB Data
            if (fspec[43] === "1") {
              ...
              schduler.addOperation(cat21.setModeSMBData(modeSMBData, rep));
              offset += 1 + 8 * rep;
            }
            // ACAS Resolution Advisory Report
            if (fspec[44] === "1") {
              var acasResolutionAdvisoryReport = message.subarray(offset, offset + 7);
              schduler.addOperation(cat21.setACASResolutionAdvisoryReport(acasResolutionAdvisoryReport));
              offset += 7;
            }
            // Receiver ID
            if (fspec[45] === "1") {
              var receiverID = message.subarray(offset, offset + 1);
              schduler.addOperation(cat21.setReceiverID(receiverID));
              offset += 1;
            }
            // Data Ages
            if (fspec[46] === "1") {
              var dataAges = message.subarray(offset, message.length);
              schduler.addOperation(cat21.setDataAges(dataAges));
              offset += len;
            }
          }
        }
