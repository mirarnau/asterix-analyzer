...
export class TargetIdentificationDecoder {
    ...
    public async decode(item : Buffer) : Promise<TargetIdentification>{
        ...
        var sti = this.decodeSti(bits);
        var data = [];
        var offset = 8;
        for (var i = 0; i < 8; i++) {
            data.push(this.decodeData(bits.slice(offset, offset + 6)));
            offset += 6;
        }

        return new TargetIdentification(sti, data.join(""));
    }

    private decodeSti(bits : string[]) : string{
        switch (bits.slice(0, 2).join("")) {
            case "00":
              return "Callsign or registration downlinked from transponder";
            case "01":
              return "Callsign not downlinked from transponder";
            case "10":
              return "Registration not downlinked from transponder";
          }
        return "Unknown";
    }

    private decodeData (bits: string[]) {
        var res = "";
