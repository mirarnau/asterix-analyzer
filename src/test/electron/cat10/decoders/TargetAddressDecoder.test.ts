...
import { TargetAddress } from '../../../../electron/cat10/valueObjects/TargetAddress';
...
test('givenValidBinaryData_WhenDecodeTargetAddress_thenCorrectValues', async () => {
    ...
    var expectedValue : string = "0x342387";
    ...
    var targetAddress : TargetAddress = (await cat10Adapter.adapt(messageList[1])).targetAddress;
   
    //Then
    expect(targetAddress).not.toBe(null);
    expect(targetAddress.value).toBe(expectedValue);
